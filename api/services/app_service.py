...
from models.app_permission import AppPermissionMember, AppPermissionSetting
...
class AppService:
    def get_paginate_apps(
        self, user_id: str, tenant_id: str, args: dict, current_account: Account | None = None
    ) -> Pagination | None:
        """
        ...
        if current_account is not None:
            filters.append(AppPermissionService.build_edit_permission_filter(current_account))
    ...
    def delete_app(self, app: App):
        ...
        :param app: App instance
        """
        db.session.query(AppPermissionMember).filter(AppPermissionMember.app_id == app.id).delete()
        db.session.query(AppPermissionSetting).filter(AppPermissionSetting.app_id == app.id).delete()
