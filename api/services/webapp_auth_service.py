...
class WebAppAuthService:
    ...
    def is_app_require_permission_check(
        ...
        if app_model and not AppPermissionService.is_public_app(app_model):
            return True
        return False
    ...
    def get_app_auth_type(cls, app_code: str | None = None, access_mode: str | None = None) -> WebAppAuthType:
        ...
        if app_code:
            ...
            if dify_config.ENTERPRISE_ENABLED:
                ...
                enterprise_auth_type = cls.get_app_auth_type(access_mode=webapp_settings.access_mode)
                if enterprise_auth_type != WebAppAuthType.PUBLIC:
                    return enterprise_auth_type
            app_model = db.session.get(App, app_id)
            if app_model and not AppPermissionService.is_public_app(app_model):
                return WebAppAuthType.INTERNAL
            return WebAppAuthType.PUBLIC
