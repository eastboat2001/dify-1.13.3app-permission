...
class AppAccessMode(Resource):
    ...
    def get(self):
        ...
        if not features.webapp_auth.enabled:
            access_mode = "private" if WebAppAuthService.is_app_require_permission_check(app_id=app_id) else "public"
            return {"accessMode": access_mode}

        res = EnterpriseService.WebAppAuth.get_app_access_mode_by_id(app_id)
        app_model = db.session.get(App, app_id)
        if res.access_mode == "public" and app_model is not None and not AppPermissionService.is_public_app(app_model):
...
class AppWebAuthPermission(Resource):
    ...
    def get(self):
        ...
        if not features.webapp_auth.enabled:
            app_model = db.session.get(App, app_id)
            account = None if user_id == "visitor" else db.session.get(Account, str(user_id))
            if account and app_model:
                account.set_tenant_id(app_model.tenant_id)
            return {"result": bool(app_model and AppPermissionService.can_use_app(account, app_model))}
        ...
        if app_model is not None and not AppPermissionService.is_public_app(app_model):
            ...
            if account:
                account.set_tenant_id(app_model.tenant_id)
            res = res and AppPermissionService.can_use_app(account, app_model)
        return {"result": res}
