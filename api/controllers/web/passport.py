...
class PassportResource(Resource):
    ...
    def get(self):
        ...
        if isinstance(app_model, App) and not AppPermissionService.is_public_app(app_model):
            console_user_decoded = decode_console_user_id(extract_access_token(request))
            if not console_user_decoded:
                raise WebAppAuthRequiredError()
            account = db.session.get(Account, console_user_decoded.get("user_id"))
            if not account:
                raise WebAppAuthRequiredError()
            account.set_tenant_id(app_model.tenant_id)
            if not AppPermissionService.can_use_app(account, app_model):
                raise WebAppAuthAccessDeniedError()
            return exchange_token_for_account_web_user(
                app_model=app_model,
                site=site,
                account=account,
                expires_unix=console_user_decoded.get("exp"),
            )
...
def decode_console_user_id(jwt_token: str | None):
    """Decode the normal Dify console login token for local private Web App URLs."""
    if not jwt_token:
        return None

    decoded = PassportService().verify(jwt_token)
    if not decoded.get("user_id"):
        raise Unauthorized("Invalid console token.")
    return decoded
...
def exchange_token_for_account_web_user(app_model: App, site: Site, account: Account, expires_unix: int | None = None):
    end_user = db.session.scalar(
        select(EndUser).where(EndUser.app_id == app_model.id, EndUser.session_id == account.email)
    )
    if not end_user:
        end_user = EndUser(
            tenant_id=app_model.tenant_id,
