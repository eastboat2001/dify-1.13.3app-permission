...
class AppPermissionPayload(BaseModel):
    edit_scope: AppEditPermissionScope = Field(..., description="App edit permission scope")
    edit_members: list[str] = Field(default_factory=list, description="Account IDs with selected edit access")
    use_scope: AppUsePermissionScope = Field(..., description="App use permission scope")
    use_members: list[str] = Field(default_factory=list, description="Account IDs with selected use access")
...
class AppPermissionResponse(ResponseModel):
    edit_scope: str
    edit_members: list[str] = Field(default_factory=list)
    use_scope: str
    use_members: list[str] = Field(default_factory=list)
    can_manage: bool
    creator_id: str
...
class AppPartial(ResponseModel):
    ...
    edit_member_count: int = 0
    use_scope: str | None = None
    use_member_count: int = 0
...
class AppListApi(Resource):
    ...
    def get(self):
        ...
        app_pagination = app_service.get_paginate_apps(current_user.id, current_tenant_id, args_dict, current_user)
        ...
        for app in app_pagination.items:
            ...
            AppPermissionService.attach_permission_summary(app)
...
class AppApi(Resource):
    ...
    def get(self, app_model):
        ...
        if not AppPermissionService.can_edit_app(
            current_user, app_model
        ) and not AppPermissionService.can_manage_app_permission(current_user, app_model):
            raise Forbidden()
        app_service = AppService()

        app_model = app_service.get_app(app_model)
        AppPermissionService.attach_permission_summary(app_model)
    ...
    def delete(self, app_model):
        """Delete app"""
        current_user, _ = current_account_with_tenant()
        if not AppPermissionService.can_manage_app_permission(current_user, app_model):
            raise Forbidden()
        app_service = AppService()
...
class AppPermissionApi(Resource):
    @console_ns.doc("get_app_permissions")
    @console_ns.doc(description="Get application permission settings")
    @console_ns.doc(params={"app_id": "Application ID"})
    @console_ns.response(200, "Success", console_ns.models[AppPermissionResponse.__name__])
    @console_ns.response(403, "Insufficient permissions")
    @setup_required
    @login_required
    @account_initialization_required
    ...
    def get(self, app_model):
        current_user, _ = current_account_with_tenant()
        can_manage = AppPermissionService.can_manage_app_permission(current_user, app_model)
        if not can_manage and not AppPermissionService.can_edit_app(current_user, app_model):
            raise Forbidden()

        setting = AppPermissionService.ensure_permission_setting(app_model)
        payload = AppPermissionResponse(
            edit_scope=setting.edit_scope.value,
            edit_members=AppPermissionService.get_member_ids(app_model, AppPermissionType.EDIT),
            use_scope=setting.use_scope.value,
            use_members=AppPermissionService.get_member_ids(app_model, AppPermissionType.USE),
            can_manage=can_manage,
            creator_id=app_model.created_by,
        )
        return payload.model_dump(mode="json"), 200

    @console_ns.doc("update_app_permissions")
    @console_ns.doc(description="Update application permission settings")
    @console_ns.doc(params={"app_id": "Application ID"})
    @console_ns.expect(console_ns.models[AppPermissionPayload.__name__])
    @console_ns.response(200, "App permissions updated", console_ns.models[AppPermissionResponse.__name__])
    @console_ns.response(403, "Insufficient permissions")
    @setup_required
    @login_required
    @account_initialization_required
    ...
    def put(self, app_model):
        current_user, _ = current_account_with_tenant()
        payload = AppPermissionPayload.model_validate(console_ns.payload or {})

        setting = AppPermissionService.update_permissions(
            app=app_model,
            account=current_user,
            edit_scope=payload.edit_scope,
            edit_member_ids=payload.edit_members,
            use_scope=payload.use_scope,
            use_member_ids=payload.use_members,
        )
...
class AppCopyApi(Resource):
    ...
    def post(self, app_model):
        ...
        app = db.session.get(App, result.app_id)
        if app is None:
            raise BadRequest("Failed to copy app")
        AppPermissionService.ensure_permission_setting(app)
        db.session.commit()
        AppPermissionService.attach_permission_summary(app)
