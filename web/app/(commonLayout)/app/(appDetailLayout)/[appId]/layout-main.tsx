...
const AppDetailLayout: FC<IAppDetailLayoutProps> = (props) => {
  ...
  const getNavigationConfig = useCallback((appId: string, isCurrentWorkspaceEditor: boolean, mode: AppModeEnum) => {
    const navConfig = [
      ...
      ...(isCurrentWorkspaceEditor
        ? [{
            name: t('appMenus.permissions', { ns: 'common' }),
            href: `/app/${appId}/permissions`,
            icon: RiLock2Line,
  ...
  useEffect(() => {
    ...
    if (!canIEditApp && (pathname.endsWith('configuration') || pathname.endsWith('workflow') || pathname.endsWith('logs') || pathname.endsWith('permissions'))) {
